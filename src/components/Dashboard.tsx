...
export function Dashboard() {
  ...
  const toggleTheme = useCallback(() => {
    setTheme((prev) => {
      ...
      setStatusAnnouncement(`Switched to ${nextTheme} mode`);
      return nextTheme;
    });
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStatusAnnouncement("Failed to refresh signals");
    } finally {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Don't trigger shortcuts if user is typing inside an input/textarea/select
    ...
    <div className="console">
      ...
      <header className="console__header" role="banner">
        ...
        <div className="console__header-right">
          ...
          <button
            ...
            data-theme-toggle
            onClick={toggleTheme}
            aria-label={theme === "dark" ? "Switch to light theme (Shortcut: T)" : "Switch to dark theme (Shortcut: T)"}
      ...
      <main id="main-content" tabIndex={-1}>
        <section className="card connection-card" aria-labelledby="connection-title">
          <div className="connection-card__head">
            ...
            <span
              ...
              aria-label={`Database status: ${databaseMode === "live" ? "Live mode" : "Mock mode"}`}
            >
        ...
        <section className="card" aria-labelledby="signals-title">
            ...
            <span className="signals__source">
              ...
              {lastUpdated && (
                <span className="signals__updated">
                  · updated{" "}
                  <time dateTime={lastUpdated.toISOString()} title={lastUpdated.toLocaleString()}>
                    {timeAgo(lastUpdated)}
                  </time>
                </span>
              )}
          ...
          {loading ? (
            <p className="muted" aria-live="polite">Loading signals…</p>
          ) : (
            <div className="table-wrap">
              <table aria-label="Signals data list">
                <caption>Active and historical database signals</caption>
                <thead>
                  <tr>
...
function SignalRow({ signal }: { signal: Signal }) {
  ...
  const intensityClamped = Math.max(0, Math.min(100, signal.intensity));
      ...
      <td className="num">
        <div className="intensity">
          <div
            ...
            role="progressbar"
            aria-valuenow={intensityClamped}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`Intensity ${intensityClamped}%`}
          >
            <div
              className="intensity__fill"
              style={{ width: `${intensityClamped}%` }}
            />
          </div>
